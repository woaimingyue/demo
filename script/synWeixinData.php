<?php
/**
 * E:\wamp\bin\php\php5.5.12>php E:\demo\index.php
...
function getToken()
{
    ...
    if (empty($token)) {
        $url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=wxfa0ca35014473abe&secret=f0718d84da9322703f1b567d477de410";
                            ...
                            if (isset($weixin['errcode'])) {
                               ...
                               db($sql);
                                ...
                                try {
                                    ...
                                    //插入数据库
                                    $sql = "insert into weixin_users (`openid`,`nickname`,`sex`,`language`,`city`,`province`,`country`,`headimgurl`,`remark`,`subscribe_time`,`groupid`,`tagid_list`) values ('{$weixin['openid']}','{$weixin['nickname']}','{$weixin['sex']}','{$weixin['language']}','{$weixin['city']}','{$weixin['province']}','{$weixin['country']}','{$weixin['headimgurl']}','{$weixin['remark']}','{$weixin['subscribe_time']}','{$weixin['groupid']}','{$weixin['tagid_list']}')";
                                    db($sql);
